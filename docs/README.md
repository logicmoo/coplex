...
| 4 | [04-rest-api.md](04-rest-api.md) | `codex_harness_server.pl`'s full REST surface, the async-run/list-summary contract a UI is meant to drive, and REST-layer security. |
...
in-process from Prolog; `codex_harness_server.pl` additionally exposes
...
graph TB
    ...
    subgraph Plugin["task_harness_pl plugin"]
        ...
        subgraph SW["swipl subprocess"]
            MAIN["codex_harness_server_main.pl\n(runnable entry point)"]
            SRV["codex_harness_server.pl\n(JSON REST facade)"]
...
2. **REST facade** (`codex_harness_server.pl` +
   `codex_harness_server_main.pl`) — a thin, security-conscious JSON
